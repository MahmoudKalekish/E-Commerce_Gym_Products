...
if (isset($_POST["category"])) {
	...
	echo "
		<div class='nav  nav-stacked'>
			<li class='active ProdCatTittle'><h4>Product Categories</h4></li>
	";
	if (mysqli_num_rows($run_query) > 0) {
		while ($row = mysqli_fetch_array($run_query)) {
			...
			echo "
					<li><a href='#' class='category ProdCat' cid='$cid'>$cat_name</a></li>
			";
		}
		echo "</div>";
	}
...
if (isset($_POST["brand"])) {
	...
	echo "
		<div class='nav  nav-stacked'>
			<li class='active BrandTittle'><h4>Brands</h4></li>
	";
	if (mysqli_num_rows($run_query) > 0) {
		while ($row = mysqli_fetch_array($run_query)) {
			$bid = $row["brand_id"];
			...
			echo "
					<li><a href='#' class='selectBrand ProdCat'  bid='$bid'>$brand_name</a></li>
			";
		}
		echo "</div>";
	}
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info' style='border-color:black;'>
								<div class='panel-heading' style='background-color:black;'>$pro_title</div>
								<div class='panel-body'>
									<img src='product_images/$pro_image' style='width:100%; height:100%;'/>
								</div>
								<div class='panel-heading' style='background-color:#c11325;'>" . CURRENCY . " $pro_price
		...
		echo "
				<div class='col-md-4'>
							<div class='panel panel-info' style='border-color:black'>
								...
								<div class='panel-heading' style='background-color:#c11325;'>$ $pro_price
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>Add To Cart</button>
								</div>
							</div>
						</div>	
		...
		if (mysqli_num_rows($query) > 0) {
			...
			if (!isset($_SESSION["uid"])) {
				...
				'<input type="hidden" name="return" value="http://localhost/project1/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/E-Commerce_Gym_Products/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/E-Commerce_Gym_Products/cancel.php"/>
